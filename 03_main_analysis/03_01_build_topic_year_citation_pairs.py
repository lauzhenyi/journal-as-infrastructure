#!/usr/bin/env python3
"""Build temporally eligible cross-journal coordination opportunities."""
...
connection.execute(
    "SET temp_directory='/private/tmp/duckdb_temporal_coordination_tmp'"
)
...
build_table(
    "temporal_coordination_members_six",
    """
    WITH clean AS (
        SELECT
            universe,
    ...
    SELECT *
    FROM clean
    WHERE clean_family_size BETWEEN 2 AND 20
    """,
)

build_table(
    "temporal_coordination_opportunities_six",
    """
    SELECT
        later.universe,
        later.focal_work_id,
    ...
    FROM temporal_coordination_members_six AS later
    ...
    GROUP BY later.universe, later.focal_work_id
    """,
)

build_table(
    "temporal_coordination_realized_six",
    """
    SELECT
        pair.universe,
        pair.focal_work_id,
...
build_table(
    "temporal_coordination_topic_year_six",
    """
...
output_path = OUTPUT_DIRECTORY / "temporal_coordination_topic_year.parquet"
connection.execute(
    f"""
    COPY temporal_coordination_topic_year_six
