#!/usr/bin/env python3
"""Build the no-upper-limit researcher coordination sensitivity panel."""
...
connection.execute(
    "SET temp_directory='/private/tmp/duckdb_no_upper_limit_researcher_tmp'"
)
...
build_table(
    "no_upper_limit_temporal_researcher_coordination_six",
    """
    WITH family AS (
        SELECT
            cohort.universe,
        ...
        INNER JOIN no_upper_limit_temporal_coordination_opportunities_six AS opportunity
...
output_path = OUTPUT_DIRECTORY / "no_upper_limit_temporal_researcher_coordination.parquet"
connection.execute(
    f"""
    COPY no_upper_limit_temporal_researcher_coordination_six
...
violation_count = connection.execute(
    """
    SELECT COUNT(*)
    FROM no_upper_limit_temporal_researcher_coordination_six
